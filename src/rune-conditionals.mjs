...
/**
 * 데미지 공식에 안 들어가지만 DPS 에는 기여하는 효과(쿨감·속도 등)를, 데미지 % 로 환산한 값.
 * 타당 데미지가 아니라 DPS 배수이므로 최종 점수에 곱한다.
 *
 * 룬별 값과 그 근거(실측 방법·왜 지금 0 인지)는 데이터에 있다
 * — UTILITY_DAMAGE_EQUIVALENT 의 percent / note / disabledReason.
 * 예전에는 그 근거가 이 주석에만 있어서, 데이터만 보는 사람은 0 이
 * "측정 결과 무의미" 인지 "일부러 끔" 인지 알 수 없었다.
 */
