...
npm test
npm run build
npm run impact                # 이 변경이 어떤 룬을 움직였는지 룬별로 훑는다
```

**`npm run impact` 의 출력은 눈으로 읽어야 한다.** 통과·실패가 없다 — 달라진 룬을 늘어놓을
뿐이고, 그중 의도한 것이 무엇인지는 사람만 안다. 예상 못 한 룬이 목록에 있으면 거기서 멈춰라.

이게 따로 있는 이유는 **테스트가 내가 지킬 생각을 한 것만 지키기 때문**이다. 골든 점수는
룬 6개짜리 세트 하나를 고정하므로 그 세트 밖의 룬이 통째로 망가져도 초록불이고, 검증기는
데이터의 모양을 볼 뿐 룬 하나가 화면에서 어떻게 보이는지는 안 본다. 실제로 그 틈으로
샜다 — 설명문을 훑어 페널티를 만들던 코드가 룬 넷에 없는 페널티를 붙였는데 점수는 1원도
안 움직였고 테스트 190개가 전부 통과했다.
...
- **'감소' 를 손해로 읽지 마라.** 좋고 나쁨은 **무엇이** 움직였는지가 정한다 — 받는 피해는
  줄면 이득이고 쿨타임도 줄면 이득이다. 이 저장소는 이걸 세 번 틀렸고 매번 다른 파일
  다른 줄이었다. 판단하는 자리는 `src/stat-polarity.mjs` 의 `isHarmful()` 하나뿐이고,
  방향은 `STAT_BETTER_WHEN`(데이터)이 스탯마다 한 번 선언한다. 다른 파일에서 방향 낱말로
  판단하면 테스트가 파일 이름과 함께 잡는다.
