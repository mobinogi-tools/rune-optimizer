...
export function validateData(root = '.', expectedFromNames = EXPECTED_FROM_NAMES) {
  ...
  for (const file of files.sort()) {
    ...
    if (!nb) err(where, 'nightBlessing 이 없다');
      ...
      checkKeys(w, nb, ['trigger', 'triggerIntervalSeconds', 'cycleSeconds', 'extendedSeconds', 'effects', 'confidence', 'note', 'evidence']);
      /* 실제로 도는 주기를 직접 적은 값. 트리거 간격에서 유도한 것보다 이쪽이 우선한다.
       * 0 이하면 나눗셈이 터지고, 밤의 축복 지속보다 짧으면 구간이 음수가 된다. */
      if (nb.cycleSeconds !== undefined && !(Number.isFinite(nb.cycleSeconds) && nb.cycleSeconds > 0)) {
        err(w, `cycleSeconds 는 0보다 큰 수여야 한다 (받은 값: ${JSON.stringify(nb.cycleSeconds)})`);
      }
      /* 밤의 축복을 늘려주는 직업 스킬이 있으면 그 초를 적는다. 음수·0 은 뜻이 없고,
       * 주기보다 길면 구간 계산이 음수로 새어 조용히 틀린 점수가 나온다(주기 검사는 평가기). */
      if (nb.extendedSeconds !== undefined && !(Number.isFinite(nb.extendedSeconds) && nb.extendedSeconds > 0)) {
        err(w, `extendedSeconds 는 0보다 큰 수여야 한다 (받은 값: ${JSON.stringify(nb.extendedSeconds)})`);
      }
  ...
  const RUNE_NAME_MAPS = ['POLLUTION_REDUCTION', 'COOLDOWN_RUNES', 'RUNE_FAMILY',
    'RUNE_CONTENT', 'DOT_APPLIER_RUNES'];
  /* 쿨감 룬은 '이 룬이 쿨감 룬이다' 라는 사실만 갖는다. 값은 룬이 아니라 사람이 정한다
   * (프로필의 cooldownRuneDamagePercent). 예전에는 룬마다 환산 %% 를 들고 있었는데,
   * 그러면 두 개를 끼었을 때 곱으로 겹치고 단위가 다른 둘(비율 vs 절대 시간)을 억지로
   * 같은 자리에 두게 된다. 대신 근거는 여기 남긴다 — 사람이 값을 정할 때 읽을 것이다. */
  for (const [name, v] of Object.entries(rc.COOLDOWN_RUNES ?? {})) {
    const where = `data/rune-conditionals.json[COOLDOWN_RUNES][${name}]`;
    if (!v || typeof v !== 'object') { err(where, '{ note } 객체여야 한다'); continue; }
    checkKeys(where, v, ['note']);
